...
def create_filter_doc():
    ...
    if len(filtered_table) == 0:
        ...
        #внесение существующих фильтров в документ
...
def Analysis_Dialog():
    ...
    def create_analysis_doc():
        ...
        if date_start != '' and date_t != '':
            ...
            table = doc.add_table(rows=len_general_regnum_day+1, cols=6)  # добавление таблицы
            table.style = 'Table Grid'  # применение стиля для таблицы
            ...
            # заполнение таблицы данными
            for i in range(1, len_general_regnum_day+1):
                for j in range(6):
    ...
    def calculates_characteristics():
        ...
        def calc_char_diff_days(date_start, date_end, regnum_day):
            '''Функция рассчитывает среднее, дисперсию и размах процентных ставок для каждого кода бумаги выбранного интервала
            ...
            for i in range(len(regnum_day)):
                ...
                if len(one_regnum_rate) == 1:
                    ...
                    variance_rate.append(variance(one_regnum_rate))

                mean_rate.append(mean(one_regnum_rate))
        ...
        global len_general_regnum_day
        len_general_regnum_day = len(general_regnum_day)
        ...
        for i in range(len(general_regnum_day)):
            ...
            ui_analysis.tableWidget.item(i, 1).setText('{:0.6f}'.format(mean_rate[i]))
            ui_analysis.tableWidget.item(i, 2).setText('{:0.10f}'.format(variance_rate[i]))
...
def Edit_Dialog():
    ...
    def sql_update_query(headers, price_cell, gain_end_cell):
        ...
        query = QSqlQuery()
        sql = 'UPDATE {} SET {} = "{}", {} = "{}" WHERE {} = "{}" AND {} = "{}" AND {} = "{}" AND {} = "{}" '. \
            format('Activ', headers[2], price_cell,
                   headers[3], gain_end_cell,
...
def get_Activ():
    '''Функция создает объект таблицу Activ'''
    global Activ
